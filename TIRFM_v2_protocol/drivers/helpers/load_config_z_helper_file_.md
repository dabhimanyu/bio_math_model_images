# Updated MATLAB Code for load_config.m
---

This is a helper file for the driver file `load_config.m`. The function is as follows.
...
%
%   The loader performs a simple sanity check on the mandatory
...
The `mFile` for the function itself is well written. But here are a few more important points to consider. 
- First of all a critical point to note.
```
% 
% CRITICAL:
% 
...
# NOTE: Verification Steps for Updated load_config.m

1. Load a preset JSON (e.g., `tirf_default.json` with new schema) via `C = load_config('configs/tirf_default.json')`; check if `C.pre_mask_preprocess.wiener.window == 5` and `C.masking.manual.threshold == 18500` (confirms integration).
2. Test merging with a partial user JSON missing fields (e.g., omit `post_mask_preprocess`); verify `C.post_mask_preprocess.enabled == true` inherits from defaults.
3. Intentionally use an invalid mode (e.g., edit JSON to `mode: 'invalid'`); ensure error throws for sanity check.
4. For non-tirf preset (e.g., `synthetic_default.json`), confirm `C.hmax_suppression.enabled == false` and `C.regional_max.rescale_before == false`.
5. Check nested overrides: Create a custom JSON overriding `masking.morphology.dilate_radius=10`; load and verify `C.masking.morphology.dilate_radius == 10` while others default.
